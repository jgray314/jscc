...
2. **Role focus and level.** Compare the posting's title and level against the profile's `role_focus` and `level_target`. Tell two distances apart:
   ...
   - *Adjacent:* the same job family one step above the target titles (e.g. a director-, head-of- or VP-titled engineering leadership posting against an engineering-manager/staff-plus profile). `role_focus` lists target titles, not the only acceptable ones, so this is a stretch, not a mismatch: deduct modestly at most and let the other factors decide, especially when comp meets or exceeds the range. With comp at or above the range, every must-have met and no other detail either way, an adjacent posting scores in the low 70s (about 70-75); score it higher only when the posting gives concrete reasons the scope fits, lower when other factors are weak.
   A title that uses the profile's own target words ("Engineering Manager", "Staff Engineering Manager", "Senior Engineering Manager", "Senior Staff Engineer") is on target, not adjacent. A people-manager posting's extracted `level` of "senior" is the extraction's floor for any manager title, not evidence the role sits below a staff-plus target.
3. **Comp target.** Compare the posting's `comp_band` against the profile's `comp_target` range. Below-range comp caps the score in the middle band (about 45-70) even if everything else matches well; comp above the profile's range is not a downside — score it as if it met the top of the range. A band that straddles the bottom of the range counts as within range when its midpoint is above the range minimum (a band of $290,000-$360,000 against a $300,000 minimum) and as below range when its midpoint is at or below it. A posting with no stated comp band is neither a bonus nor a penalty on this factor alone.
4. **Must-haves.** Check the posting (both the structured skills list and the raw text — a must-have like "remote or hybrid" is about `remote_policy`, not `must_have_skills`) against every entry in the profile's `must_haves`. Missing one must-have caps the score in the low-to-middle band (about 15-50), below a comp shortfall alone; missing several pushes it toward the bottom. A must-have the posting says nothing about is unknown, not missing: it neither caps nor adds. Treat a must-have as missing only when the posting contradicts it (onsite-only against "remote or hybrid", a legacy-only stack against "modern tooling").
