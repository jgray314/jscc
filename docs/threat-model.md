...
| T8 | A code path sends text to the model without sanitizing it | The send boundary verifies an HMAC-authenticated payload, so a forged or mutated payload is refused. `tests/test_llm_egress.py` fails if a module outside the known four calls the model client, if a stage verifies before it sanitizes or hands the client anything not read from the verified payload, or if a stage still reaches its client after the boundary refuses. Each check was confirmed by injecting the regression and watching it fail (ADR-005, second addendum) | Enforced by inspection, not by type: the client's `complete` still takes three plain strings, so a new caller written to look like the existing four would pass. An authenticated argument type is the stronger fix and is the trigger recorded in the ADR. Until 2026-09-20 nothing enforced this at all, although ADR-005's trigger for it had fired when the scorer landed. **Controlled, by test** |
