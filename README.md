...
| **Phase C — fit scoring** | Eval suite (C1), prompt + client plumbing (C2a), and manual-capture validation (C2b — 84% on round 1, then 64% on a recapture that exposed rubric gaps; the revised prompt scored 27/28 on its first round, a rate the prompt and fixtures were tuned toward; see CHANGELOG). Cost/latency reporting (C3): `jscc costs` prints per-feature cost, latency percentiles, and flags any call whose recorded cost no longer matches its model's published rate. | — |
