...
1. **An eval result reported as a band, not a point.** Extraction has been measured several times and landed anywhere from 56% to 89% as the prompt changed and the suite grew from 33 to 36 cases; the repo lists every round instead of picking one. The case count is sized against the statistical noise it leaves, and the reasoning is written down. → [evals/README.md](evals/README.md) (case sizing and per-field grading rules), [`jscc/evals.py`](jscc/evals.py) (the harness, `PASS_THRESHOLD` in code).
...
Then open http://127.0.0.1:8000. No `ANTHROPIC_API_KEY` is required for browsing — it's only hit if you submit the DLQ resolve form (`/dlq/{id}/resolve`), which runs a real extraction call.
...
| **Phase B — ingestion + extraction** | Eval suite, extraction prompt validated against real model output (32/36, 89%, on the latest round; see below), fetcher + Playwright fallback + DLQ, paste-only path, three-value exit contract, ruff lint/format gate. Phase B → C gate fully closed. | — |
| **Phase C — fit scoring** | Eval suite (C1), prompt + client plumbing (C2a), and manual-capture validation (C2b — 84% on round 1, then 64% on a recapture that exposed rubric gaps; the revised prompt scored 27/28 in each of two rounds, a rate the prompt and fixtures were tuned toward; see CHANGELOG). Cost/latency reporting (C3): `jscc costs` prints per-feature cost, latency percentiles, and flags any call whose recorded cost no longer matches its model's published rate. | — |
...
689 pytest cases. Lint and format enforced via ruff (see Development, above).
