...
jscc/           library code
  ...
  composition.py  the compose_followup interface (D10 step 2A) -- stub until D4
  report.py     staleness detector + funnel counts
  cli.py        click entry point (ingest, dlq list, resolve-dlq, route, ...)
tests/          pytest suite (457 tests)
...
457 pytest cases. Lint and format enforced via ruff (see Development, above).
