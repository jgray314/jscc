...
Run: `python -m jscc eval routing`. D2a landed the real prompt (Haiku, per D10 — reuses `EXTRACTION_MODEL`'s id) and full `--record`/`--replay`/`--manual`/`--min-pass-rate` parity with `fit_scoring`, but no `ANTHROPIC_API_KEY` is configured for this project — same constraint B2/C2 hit — so today it runs end-to-end against `StubRoutingClient`. Unlike the other two stubs, `StubRoutingClient`'s fixed answer is a deliberate one, not an arbitrary placeholder: per D10's own bias, it always answers `non_routine` — the honestly correct "safe when uncertain" behavior for an unconfigured router, not just a stand-in. That means the stub trivially clears gate (2) (it never says "routine," so there is nothing to be a false-routine case) while failing gate (1) at 50% (6/12 — every non_routine-expected case passes, every routine-expected case doesn't, since the stub can't tell them apart).
